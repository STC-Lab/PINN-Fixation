...
X_data_tensor_train,X_data_tensor_val,T_data_tensor_train,T_data_tensor_val,U_data_tensor_train,U_data_tensor_val,X_physics_tensor_train,X_physics_tensor_val,T_physics_tensor_train,T_physics_tensor_val,U_physics_tensor_train,U_physics_tensor_val= dataprocessing.split_data(X_data_tensor,T_data_tensor,U_data_tensor,X_physics_tensor,T_physics_tensor,U_physics_tensor, Nf_val)
...
while loss_v > theta:
    ...
    physics_input = [X_physics_tensor_train,T_physics_tensor_train]
    physic_output = pinn(physics_input)
    dudt = torch.autograd.grad(physic_output, T_physics_tensor_train, torch.ones_like(physic_output), create_graph=True)[0]
    dudx = torch.autograd.grad(physic_output, X_physics_tensor_train, torch.ones_like(physic_output), create_graph=True)[0]
    d2udx2 = torch.autograd.grad(dudx, X_physics_tensor_train, torch.ones_like(dudx), create_graph=True)[0]
